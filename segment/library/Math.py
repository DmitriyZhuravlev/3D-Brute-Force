...
def rotation_matrix(yaw, pitch=0, roll=0): #pitch=np.radians(4), roll=np.radians(0)):
    tx = roll
    ty = yaw
    tz = pitch
...
def calc_location_test(dimension, proj_matrix, box_2d, theta_ray, alpha):
    ...
    for constraint in constraints:
        ...
        try:
            ...
            corners = create_corners(dimension, loc, R=R)
            box_3d = []
            ...
            over_error = 0
            for p in box_3d:
                if p[0] < xmin: over_error += xmin - p[0]
                if p[0] > xmax: over_error += p[0] - xmax
                if p[1] < ymin: over_error += ymin - p[1]
                if p[1] > ymax: over_error += p[1] - ymax
            # found a better estimation
            # TODO add internal only ?
            if loc is not None and not np.isnan(np.min(loc)) and over_error < best_over_error: #error < best_error:
                #count += 1 # for debugging
                #print("updating")
                best_loc = loc
                best_error = error
                best_X = X_array
                best_over_error = over_error
    ...
    return best_loc, best_X, [best_over_error], best_error # best_error #[best_over_error]
...
def create_bottom_corners(dimension, location=None, R=None):
    dx = dimension[2] / 2
    #dy = dimension[0] / 2
    dz = dimension[1] / 2
...
def calc_location(dimension, proj_matrix, box_2d, alpha, theta_ray):
    ...
    for constraint in constraints:
        ...
        try:
            ...
            # found a better estimation
            if loc is not None and not np.isnan(np.min(loc)) and error < best_error:
                #count += 1 # for debugging
                #print("updating")
                best_loc = loc
                best_error = error
    ...
    return best_loc, best_X,  best_error
