...
class Dataset(data.Dataset):
    def __init__(self, path, num, bins=2, overlap=0.1):

        self.top_label_path = path + "/label_" + str(num) + "/"
        self.top_img_path = path + "/image_" + str(num) + "/"
        ...
        #print(path + "/label_" + str(num) + "/")
        #print(path + "/image_" + str(num) + "/")

        # TODO: which camera cal to use, per frame or global one?
        #self.proj_matrix = get_P(os.path.abspath(os.path.dirname(os.path.dirname(__file__)) + '/camera_cal/calib_cam_to_cam.txt'), num)
    ...
    def __getitem__(self, index):
        ...
        proj_matrix = get_calibration_cam_to_image(calib_path, self.num)
    ...
    def format_label(self, line):
        ...
        try:
            ...
            Alpha = 0 #line[3] # what we will be regressing
            Ry = 0 #line[14]
        ...
        with open(label_path, 'r') as f:
            for line in f:
                ...
                try:
                    ...
                    Alpha = 0 #line[3] # what we will be regressing
                    Ry = 0 #line[14]
                    top_left = (0,0) #(int(round(line[4])), int(round(line[5])))
                    bottom_right = (0, 0)#(int(round(line[6])), int(round(line[7])))
                    Box_2D = [top_left, bottom_right]
    
                    Dimension = [0,0,0]#[line[8], line[9], line[10]] # height, width, length
                    Location = [0,0,0] #[line[11], line[12], line[13]] # x, y, z
                    Location[1] -= Dimension[0] / 2 # bring the KITTI center up to the middle of the object
...
class DetectedObject:
    def __init__(self, img, detection_class, box_2d, proj_matrix, label=None):
        ...
        #self.proj_matrix = get_calibration_cam_to_image(proj_matrix, num)
